  ...
  async execute(message, args, client) {
    ...
    if (!tournament) {
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          ...
          '**Join words:** `lol` • `cry` • `bet` • `run` • `enchant`'
        )
        .setTimestamp();
    ...
    const participantList =
      tournament.participants.length > 0
        ? tournament.participants.map((p, i) => `${i + 1}. ${p.username}`).join('\n')
        : 'No participants yet';
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        {
          name: '📊 Status',
          value: tournament.phase === 'waiting' ? '⏳ Waiting for players' : '⚔️ Battle in progress',
          inline: true,
        },
        {
          name: '👥 Participants',
